# -*- coding: utf-8 -*-
"""
This file is responsible for the automatic marking of Assignment 1, 2 and 3.
...
class Solution(Generator):
    ...
    def __init__(self, ID=None, converted_file=None):
        ...
        if self.mode == 'batch':
            ...
            self.file_handler = logging.FileHandler(os.path.join(
                self.dirname, '{}_marking.log').format(self.student_name))
    ...
    def mark(self, master_dir=None, new_call=False, mark_as_null=False):
        ...
        if self.mode == 'unit_test':
            ...
            # This puts assignment2_student_marks.txt into the directory of
            # master.py which calls the function
            marks_filepath = os.path.join(
                master_dir, "Assignment{}_student_marks.txt".format(self.assignment_num))
            # This puts assignment2_feedback.txt into the directory of master.py which
            # calls the function
            feedback_filepath = os.path.join(
                master_dir, "Assignment{}_feedback.txt".format(self.assignment_num))
    ...
    def task2(self):
        ...
        if len(lines) > 3:
            running_comment += (" There appear to be more than 3 traces in the plot (if you "
                                ...
                                "for example an `divivsion by zero'). "
...
class Solution3(Solution):
    ...
    def task4(self):
        ...
        tracks=[40] exists -> 1 marks
        my_rocket.successful_landing_counter >= 0.7*40 -> 3 marks
        ...
        # check if 40 tracks exist in variable tracks
        if hasattr(self.attempt, 'tracks'):
            student_tracks = self.attempt.tracks
            if type(student_tracks) in [list,tuple] and len(student_tracks) >= 40:
                running_comment += " We found the list 'tracks' with at least 40 rocket tracks."
                running_mark += 1
                faults = 0
                for track in student_tracks[0:39]:
                    t = track[:, 0]
                    #if len(t) < 300:
                ...
                if not faults == 0:
                    ...
                    running_comment += " You successfully landed {} out of {} attempts.".format(landings, len(student_tracks))
                    rate = landings/len(student_tracks)
                    if rate >= 0.7:
                        running_mark += 3
                    elif rate >= 0.5:
                        running_mark += 2
                ...
                running_comment += " We could not find a list 'tracks' with at least 40 rocket tracks."
